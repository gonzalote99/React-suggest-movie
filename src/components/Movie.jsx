...
function Movie({name, pt_name, cover, yt_link, imbd_classification, description, year, tags})  {
  return(
    ...
    <p className='pl-2 text-4xl font-bold text-white'>{name}</p>
      ...
      <div className='flex justify-center items-center flex-end'>
        ...
        <p className='pl-2 text-4xl font-bold text-white'>{imbd_classification}/10</p>
      </div>
    </div>
      ...
      <div className='flex flex-row p-2 my-1'>
        {tags.map((tag) => {
       return <div className='w-fit px-3 mr-2 rounded-fill border text-white'>{tag}</div>
        })}
      </div>
      ...
      <div className='w-full sm:w-2/3 px-2 text-white'>{description}</div>
        ...
        <button className='bg-gray-200 p-2 w-full rounded-lg' onClick={() => window.location.reload(false)}>suggest a movie</button>
        </div>
